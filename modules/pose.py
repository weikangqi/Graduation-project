    ...
    def draw(self, img):
        ...
        for part_id in range(len(BODY_PARTS_PAF_IDS) - 2):
            ...
            if global_kpt_a_id != -1:
                ...
                Location = x_a,y_a
                l[kpt_a_id] = Location
                cv2.circle(img, (int(x_a), int(y_a)), 3, [0,255,0], -1)
            ...
            if global_kpt_b_id != -1:
                ...
                Location = x_b,y_b
                l[kpt_b_id] = Location
                cv2.circle(img, (int(x_b), int(y_b)), 3, [0,0,255], -1)
                
            if global_kpt_a_id != -1 and global_kpt_b_id != -1:
                cv2.line(img, (int(x_a), int(y_a)), (int(x_b), int(y_b)), [255,255,0], 2)

        for i in kp:
            if l.get(i,None) == None:
                l[i] = (0,0)
        l.pop(6,None)
        l.pop(3,None)
        for i in l:
